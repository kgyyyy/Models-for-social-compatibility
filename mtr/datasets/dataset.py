...
class DatasetTemplate(torch_data.Dataset):
    def __init__(self, dataset_cfg=None, training=None, logger=None):
        super().__init__()
    ...
    def collate_batch(self, batch_list):
        ...
        for key, val_list in key_to_list.items():
            if key in ['obj_trajs', 'obj_trajs_mask', 'map_polylines', 'map_polylines_mask', 'map_polylines_center',
                       ...
                       'obj_risk_future_state']:
                val_list = [torch.from_numpy(x) for x in val_list]
                input_dict[key] = common_utils.merge_batch_by_padding_2nd_dim(val_list)
            elif key in ['scenario_id', 'if_crash', 'crash_time', 'center_objects_type']:
                input_dict[key] = np.concatenate(val_list, axis=0)
            elif key in ['track_index_to_predict', 'center_objects_world', 'center_gt_trajs', 'center_gt_trajs_mask',
                         'center_gt_final_valid_idx', 'center_gt_trajs_world', 'center_gt_risk', 'target_gt_trajs',
                         'target_gt_trajs_world', 'center_gt_risks']:
                val_list = [torch.from_numpy(x) for x in val_list]
                ...
                # except:
                #     a=1
            elif key in ['center_objects_velo_world']:
                pass
            else:
                print('Error: ', key)
